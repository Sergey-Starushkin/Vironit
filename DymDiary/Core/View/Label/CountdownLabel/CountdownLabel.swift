...
@objc public protocol CountdownLabelDelegate {
    @objc optional func countdownStarted(label: CountdownLabel)
    @objc optional func countdownPaused()
    @objc optional func countdownFinished(label: CountdownLabel)
    @objc optional func countdownCancelled()
    @objc optional func countingAt(label: CountdownLabel, counted: TimeInterval, remaining: TimeInterval)
...
public class CountdownLabel: LTMorphingLabel {
    ...
    @objc func updateLabel() {
        // delegate
        countdownDelegate?.countingAt?(label: self, counted: timeCounted, remaining: timeRemaining)
        ...
        if endOfTimer {
            ...
            countdownDelegate?.countdownFinished?(label: self)
            dispose()
            completion?()
        }
    }
...
extension CountdownLabel {
    public func start(completion: ( () -> () )? = nil) {
        ...
        // set delegate
        countdownDelegate?.countdownStarted?(label: self)
    }
...
extension CountdownLabel {
    ...
    func surplusTime(_ to1970Date: Date) -> String {
        ...
        if let second = comp.second ,let _ = timeFormat.range(of: "s"){
            labelText = labelText.replacingOccurrences(of: "s", with: String.init(format: "%ld", second))
        }
        return labelText
    }
